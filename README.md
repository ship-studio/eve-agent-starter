...
**A live setup checklist on the home screen.** It reads your env and `agent/channels/`
at request time, so it shows what's actually left to do rather than a generic list: step 1
ticks off once a model credential is present, step 2 once you've added a channel beyond the
default `eve.ts`. Lives in `app/_components/setup-checklist.tsx`; delete that file and the
`setup` prop in `app/page.tsx` once you don't need it.
